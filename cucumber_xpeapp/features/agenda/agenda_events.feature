...
Scenario: Get an event by id
  Given the WordPress JWT API is available
  When I fetch the event with id 1
  Then I receive an event detail

Scenario: Create a new event
  Given the WordPress JWT API is available
  When I create an event with title "Test Event", date "2027-01-13 00:00:00", type_id "1"
  Then I receive a confirmation of event creation

Scenario: Update an event
  Given the WordPress JWT API is available
  When I update event with id 2 to title "Updated Event", date "2027-01-14 00:00:00", type_id "1"
  Then I receive a confirmation of event update

Scenario: Delete an event
  Given the WordPress JWT API is available
  When I delete event with id 2
